...
pub fn (mut g Gen) write_fn_typesymbol_declaration(sym ast.TypeSymbol) {
	...
	if !info.has_decl && (not_anon || is_fn_sig) && !func.return_type.has_flag(.generic)
		...
		g.type_definitions.write_string('typedef ${ret_typ} (*${fn_name})(')
...
fn (mut g Gen) enum_decl(node ast.EnumDecl) {
	...
	packed_attribute := if node.typ != ast.int_type {
...
fn (mut g Gen) map_init(node ast.MapInit) {
	...
	if size > 0 {
		...
		if node.has_update_expr {
			...
			g.writeln('builtin__new_map_init(${hash_fn}, ${key_eq_fn}, ${clone_fn}, ${free_fn}, ${size}, sizeof(${key_typ_str}), sizeof(${effective_typ_str}),')
		}
		...
		g.writeln('builtin__new_map(sizeof(${key_typ_str}), sizeof(${value_typ_str}), ${hash_fn}, ${key_eq_fn}, ${clone_fn}, ${free_fn})')
	}
	if g.is_shared {
		g.write('}, sizeof(${shared_styp}))')
	} else if is_amp {
...
fn (mut g Gen) type_default_impl(typ_ ast.Type, decode_sumtype bool) string {
	...
	match sym.kind {
		...
		.array {
			...
			init_str := 'builtin____new_array(0, 0, sizeof(${elem_type_str}))'
		...
		.map {
			...
			hash_fn, key_eq_fn, clone_fn, free_fn := g.map_fn_ptrs(key_sym)
			init_str := 'builtin__new_map(sizeof(${g.styp(info.key_type)}), sizeof(${g.styp(info.value_type)}), ${hash_fn}, ${key_eq_fn}, ${clone_fn}, ${free_fn})'
			if typ.has_flag(.shared_f) {
