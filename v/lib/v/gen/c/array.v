...
fn (mut g Gen) array_init_with_fields(node ast.ArrayInit, elem_type Type, is_amp bool, shared_styp string,
	...
	if node.has_index {
		...
		if is_default_array {
			g.write('builtin____new_array_with_array_default(')
		} else if is_default_map {
			g.write('builtin____new_array_with_map_default(')
		} else {
			g.write('builtin____new_array_with_default(')
		}
		if node.has_len {
	...
	if is_default_array {
		...
		g.write('builtin____new_array_with_multi_default(')
	} else {
		g.write('builtin____new_array_with_default(')
	}
	if node.has_len {
...
fn (mut g Gen) gen_array_map(node ast.CallExpr) {
	...
	has_infix_left_var_name := g.write_prepared_tmp_value(past.tmp_var, node, ret_styp,
		'{0}')
	if left_is_array {
		g.writeln('${past.tmp_var} = builtin____new_array(0, ${past.tmp_var}_len, sizeof(${ret_elem_styp}));\n')
	}
	...
	if left_is_array {
		g.writeln2(';', 'builtin__array_push((array*)&${past.tmp_var}, &${tmp_map_expr_result_name});')
	} else {
...
fn (mut g Gen) gen_array_filter(node ast.CallExpr) {
	...
	g.writeln('${past.tmp_var} = builtin____new_array(0, ${past.tmp_var}_len, sizeof(${elem_type_str}));\n')
	...
	g.writeln2(') {', '\tbuiltin__array_push((array*)&${past.tmp_var}, &${var_name});')
	g.writeln('}')
	g.indent--
	g.writeln('}')
	if !is_embed_map_filter {
...
fn (mut g Gen) gen_array_insert(node ast.CallExpr) {
	...
	if is_arg2_array {
		g.write('builtin__array_insert_many(${addr}')
	} else {
		g.write('builtin__array_insert(${addr}')
	}
	g.expr(node.left)
	g.write(', ')
...
fn (mut g Gen) gen_array_prepend(node ast.CallExpr) {
	...
	addr := if node.left_type.is_ptr() { '' } else { '&' }
	if is_arg_array {
		g.write('builtin__array_prepend_many(${addr}')
	} else {
		g.write('builtin__array_prepend(${addr}')
	}
	g.expr(node.left)
	if is_arg_array {
		g.write(', ')
