...
fn (mut g Gen) gen_json_for_type(typ ast.Type) {
	...
	if is_json_prim(sym.name) && !utyp.has_flag(.option) && !typ.is_ptr() {
		return
	}
	g.json_types << utyp
}
...
${enc_fn_dec} {
\tcJSON *o;')
		if is_json_prim(sym.name) && utyp.is_ptr() {
			...
			if is_json_prim(g.styp(parent_typ)) {
				if utyp.has_flag(.option) {
			...
			&& (is_json_prim(g.styp(utyp.clear_flag(.option))) || sym.info !is ast.Struct) {
			g.gen_option_enc_dec(utyp, mut enc, mut dec)
		} else {
			enc.writeln('\to = cJSON_CreateObject();')
...
fn (mut g Gen) gen_sumtype_enc_dec(utyp ast.Type, sym ast.TypeSymbol, mut enc strings.Builder, mut dec strings.Builder,
	...
	for variant in info.variants {
		...
		at_least_one_prim = at_least_one_prim || is_json_prim(variant_typ)
		...
		$if json_no_inline_sumtypes ? {
			...
			if variant_sym.name == 'time.Time' {
				...
				dec.writeln('\t\t\t}')
			} else if !is_json_prim(variant_typ) && variant_sym.kind != .enum {
	...
	for field in info.fields {
		...
		if is_raw {
			...
			if is_json_prim(field_type) {
				...
				gen_js_get_opt(dec_name, field_type, styp, tmp, name, mut dec, is_required)
				dec.writeln('\tif (jsonroot_${tmp}) {')
				if is_json_prim(g.styp(field.typ.clear_option_and_result())) {
...
fn (mut g Gen) decode_array(utyp ast.Type, value_type ast.Type, fixed_array_size int, ret_styp string) string {
	...
	if utyp.has_flag(.option) {
		if fixed_array_size > -1 {
			...
			array_element_assign += 'builtin__array_push((array*)&res.data, &val);'
			res_str += 'builtin___option_ok(&(${g.base_type(utyp)}[]) { builtin____new_array(0, 0, sizeof(${styp})) }, (${option_name}*)&res, sizeof(${g.base_type(utyp)}));'
		...
		if is_array_fixed_val {
			...
			array_element_assign += 'builtin__array_push((array*)&res, &val);'
			res_str += 'res = builtin____new_array(0, 0, sizeof(${styp}));'
			array_free_str += 'builtin__array_free(&res);'
		}
	}

	mut s := ''
	if is_json_prim(styp) {
