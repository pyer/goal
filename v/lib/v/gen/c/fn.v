...
fn (mut g Gen) gen_fn_decl(node &ast.FnDecl, skip bool) {
	...
	*/
	...
	if g.inside_c_extern {
		...
		visibility_kw := if g.cur_concrete_types.len > 0 && g.pref.build_mode == .build_module {
			'static '
		} else {
			''
		}
...
fn (mut g Gen) call_expr(node ast.CallExpr) {
	...
	mut cur_line := if !g.inside_curry_call && is_gen_or_and_assign_rhs { // && !g.is_autofree {
		// `x := foo() or { ...}`
...
fn (mut g Gen) gen_array_method_call(node ast.CallExpr, left_type ast.Type, left_sym ast.TypeSymbol) bool {
	match node.kind {
		...
		.first, .last, .pop_left, .pop {
			...
			g.write('(*(${return_type_str}*)builtin__array_${node.name}(')
			...
			if variadic_count > 0 {
				if node.is_c_variadic {
					...
					if variadic_count == 1
						...
						g.write('builtin__new_array_from_c_array(${variadic_count}, ${variadic_count}, sizeof(${elem_type}), _MOV((${elem_type}[${variadic_count}]){')
						for j in arg_nr .. args.len {
...
fn call_convention_attribute(cconvention string) string {
	return '__attribute__((${cconvention})) '
}
...
fn (mut g Gen) write_fntype_decl(fn_name string, info ast.FnType, nr_muls int) {
	...
	g.write('${ret_styp} (${'*'.repeat(nr_muls + 1)}${fn_name}) (')
