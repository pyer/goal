		...
		match field.expr {
			ast.ArrayInit {
				...
				if field.expr.is_fixed && !field.expr.has_index
					...
					g.const_decl_init_later(field.mod, name, const_name, field.expr, field.typ, false)
				}
			}
			ast.StringLiteral {
				val := g.expr_string(field.expr)
			...
			ast.CallExpr {
				if field.expr.return_type.has_flag(.option)
					...
					g.const_decl_init_later(field.mod, name, const_name, field.expr, field.typ, unwrap_opt_res)
				...
				use_cache_mode := g.pref.build_mode == .build_module
				if !use_cache_mode {
				...
				if field.is_simple_define_const() {
					...
					g.const_decl_init_later(field.mod, name, const_name, field.expr, field.typ, should_surround)
					...
					g.const_decl_init_later(field.mod, name, const_name, field.expr, field.typ, has_unwrap_opt_res)
				} else {
					g.const_decl_init_later(field.mod, name, const_name, field.expr, field.typ, true)
				}
			}
		}
	}
...
fn (mut g Gen) global_decl(node ast.GlobalDecl) {
	...
	visibility_kw := if g.pref.build_mode == .build_module && g.module_built != node.mod {
//		&& !util.should_bundle_module(node.mod)
	...
	should_init := g.pref.build_mode != .build_module || (g.pref.build_mode == .build_module && g.module_built == node.mod)
	mut attributes := ''
